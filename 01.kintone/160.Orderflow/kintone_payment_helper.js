...
(function () {
     ...
     */
    function buildMailPreview(record, todayRow, allTodayRows, loginEmail, requesterEmail) {
        // レコード直下のフィールド
        var to = getFieldValue(record, CONFIG.MAIL_TO_FIELD);
        // CC: 支払い依頼者メール + ログインユーザーメール + 固定アドレス（重複除去）
        var ccParts = [];
        if (requesterEmail) ccParts.push(requesterEmail);
        if (loginEmail) ccParts.push(loginEmail);
        ccParts.push(CONFIG.FIXED_CC_ADDRESS);
        var ccSeen = {};
        ccParts = ccParts.filter(function (addr) {
            if (!addr || ccSeen[addr]) return false;
            ccSeen[addr] = true;
            return true;
        });
        var cc = ccParts.join(', ');
        ...
        // 当日行が複数ある場合は個別値ではなく CSV 添付への誘導文言にする
        var multipleRows = (allTodayRows && allTodayRows.length > 1);
        if (multipleRows) {
            costCode   = '添付ご参照ください';
    ...
    function showMailPreview() {
        ...
        kintone.api(kintone.api.url('/k/v1/record', true), 'GET', {
            ...
            // ユーザーコードを収集: ログインユーザー + 支払い依頼者
            var loginUser = kintone.getLoginUser();
            var userCodes = [];
            var loginCode = (loginUser && loginUser.code) ? loginUser.code : '';
            if (loginCode) userCodes.push(loginCode);

            // 支払い依頼者のユーザーコードを取得
            var requesterCode = '';
            var reqField = record[CONFIG.REQUESTER_FIELD];
            if (reqField && reqField.value) {
                var reqVal = reqField.value;
                if (Array.isArray(reqVal) && reqVal.length > 0) {
                    requesterCode = reqVal[0].code || '';
                } else if (typeof reqVal === 'object' && reqVal.code) {
                    requesterCode = reqVal.code;
                }
            }
            if (requesterCode && userCodes.indexOf(requesterCode) === -1) {
                userCodes.push(requesterCode);
            }

            if (userCodes.length > 0) {
                return kintone.api('/v1/users.json', 'GET', { codes: userCodes })
                    .then(function (userResp) {
                        var loginEmail = '';
                        var requesterEmail = '';
                        if (userResp.users) {
                            for (var ui = 0; ui < userResp.users.length; ui++) {
                                var u = userResp.users[ui];
                                if (loginCode && u.code === loginCode) {
                                    loginEmail = u.email || '';
                                }
                                if (requesterCode && u.code === requesterCode) {
                                    requesterEmail = u.email || '';
                                }
                            }
                        }
                        console.log('[payment-helper] ログインユーザーメール:', loginEmail);
                        console.log('[payment-helper] 支払い依頼者メール:', requesterEmail);
                        doShowMailPreview(record, loginEmail, requesterEmail);
                    }).catch(function (uerr) {
    ...
    function doShowMailPreview(record, ccEmail, requesterEmail) {
        var todayRows = getTodayRows(record);
        ...
        var mail = buildMailPreview(record, firstRow, todayRows, ccEmail, requesterEmail);
